...
import { DIASPORA_SNAPSHOT } from "../data/populations";
    ...
    <div className="panel mr-3 mt-3 w-[320px] flex-shrink-0 rounded-xl px-3 py-2 text-[11px]">
      ...
      <div className="mt-1 grid grid-cols-2 gap-x-3 gap-y-0.5 text-[10.5px]">
        {DIASPORA_SNAPSHOT.map((d) => (
          <div key={d.country} className="flex items-baseline justify-between gap-1 leading-tight">
            <span className="text-ricepaper/75">{d.country}</span>
            <span className="font-display text-oldgold">
              {(d.teochewCount / 10_000).toFixed(0)} 万
            </span>
          </div>
        ))}
      </div>
